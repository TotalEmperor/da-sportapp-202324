...
export default function SignIn() {
  ...
  // State if password is visible or not
  const [showPassword, setShowPassword] = useState(false);
  // NextJs router object for navigation
  ...
  useEffect(() => {
    if (user && user.emailVerified) {
      //If the user is already set and the Email verified, it redirects the User to the Workout Tab
      router.push("/workout")
    }
  }, [user]);

  // Handle SignIn and authentication at Firebase
  const handleSignIn = async (event) => {
    event.preventDefault()

    const {result, error} = await signIn(email, password);

    // If SignIn was unsuccessful, it returns an error message
    if (error) {
      return console.log(error)
    }

    // If SignIn was successful, it redirects you to the Workout Tab
