...
export default function SetComponent(data: any) {
    return (
        <>
            <div className="rounded-xl border-2 border-[#9a9d93] w-[40rem] my-3">
                <div className="w-full justify-center flex-col mx-auto flex px-4 pt-8 py-4">
                    <div className="flex w-fit flex-row min-h-fit">
                        <span className="left-auto text-[1.8rem] font-bold me-6">{data.data[0]}</span>
                        {Array.from({length: parseInt(data.data[Object.keys(data.data)[1]]["stars"])}, (_, i) => (
                    ...
                    <div className="text-[1rem] font-bold">
                        <span>Time:{data.data[Object.keys(data.data)[1]]["time"]}</span>
                    </div>
                </div>
            </div>
        </>
